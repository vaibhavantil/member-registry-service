...
public class MemberAggregate {
    ...
    @CommandHandler
    void authAttempt(SwedishBankIdAuthenticationAttemptCommand command) {
        ...
        if (this.status == MemberStatus.INITIATED || this.status == MemberStatus.ONBOARDING) {
            ...
            applyChain = apply(new SSNUpdatedEvent(this.id, ssn, SSNUpdatedEvent.Nationality.SWEDEN));
    ...
    void startOnboardingWithSSNCommand(StartOnboardingWithSSNCommand command) {
        if (this.status == MemberStatus.INITIATED || this.status == MemberStatus.ONBOARDING) {
            apply(new OnboardingStartedWithSSNEvent(this.id, command.getSsn(), SSNUpdatedEvent.Nationality.SWEDEN));
            getBirthdateFromSwedishSsnAndApplyEvent(command.getSsn(), null);
    ...
    @CommandHandler
    void bankIdSignHandler(SwedishBankIdSignCommand cmd) {
        if (cmd.getPersonalNumber() != null
            && !Objects.equals(this.member.getSsn(), cmd.getPersonalNumber())) {
            apply(new SSNUpdatedEvent(this.id, cmd.getPersonalNumber(), SSNUpdatedEvent.Nationality.SWEDEN));
        }
        getBirthdateFromSwedishSsnAndApplyEvent(cmd.getPersonalNumber(), null);
    ...
    @CommandHandler
    public void on(MemberSimpleSignedCommand cmd) {
        String identification = cmd.getNationalIdentification().getIdentification();
        Nationality nationality  = cmd.getNationalIdentification().getNationality();

        apply(new MemberSimpleSignedEvent(cmd.getId(), identification, Nationality.Companion.toMemberSimpleSignedEventNationality(nationality), cmd.getReferenceId()));
        apply(new SSNUpdatedEvent(this.id, identification, Nationality.Companion.toSSNUpdatedEventNationality(nationality)));
    }
    ...
    @CommandHandler
    public void on(UpdateSwedishWebOnBoardingInfoCommand cmd) {
        ...
        if (cmd.getSSN() != null
            ...
            apply(new SSNUpdatedEvent(this.id, cmd.getSSN(), SSNUpdatedEvent.Nationality.SWEDEN));
            getBirthdateFromSwedishSsnAndApplyEvent(cmd.getSSN(), null);
        }
    ...
    public void on(UpdateSSNCommand cmd) {
        ...
        apply(new SSNUpdatedEvent(cmd.getMemberId(), cmd.getSSN(), Nationality.Companion.toSSNUpdatedEventNationality(cmd.getNationality())));
        getBirthdateFromSwedishSsnAndApplyEvent(cmd.getSSN(), null);
    }
