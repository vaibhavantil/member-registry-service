...
public class MailController {
  ...
  public MailController(
      ...
      @Value("${hedvig.websiteBaseUrl:https://www.hedvig.com}") String webSiteBaseUrl,
  ...
  public String sendMail(@RequestBody SendSignupRequest request)
    ...
    helper.setText(
        signupMail
            .replace("{TOKEN}", request.token.toString())
            .replace("{BASE_URL}", webSiteBaseUrl),
        true);
  ...
  public String sendOnboardedActiveTodayMail(@RequestBody SendOnboardedActiveTodayRequest request)
    ...
    val templatedMail = onboardedTodayMail.replace("{NAME}", request.getName());
  ...
  public String sendOnboardedActiveLaterMail(@RequestBody SendOnboardedActiveLaterRequest request)
      throws MessagingException {
    val message = mailSender.createMimeMessage();
    val helper = new MimeMessageHelper(message, true, "UTF-8");
    helper.setSubject("Välkommen till Hedvig! 🙌");
    ...
    val templatedMail = onboardedLaterMail.replace("{NAME}", request.getName());
  ...
  public String sendActivated(@RequestBody SendActivatedRequest request) throws MessagingException {
    ...
    helper.setFrom("\"Hedvig\" <hedvig@hedvig.com>");
    helper.setTo(request.getEmail());
    val templatedMail = activatedMail.replace("{NAME}", request.getName());
    helper.setText(templatedMail, true);
    helper.addInline("image002.jpg", signatureImage);
  ...
  public String sendWaitIsOverMail(@RequestBody SendWaitIsOverRequest request)
      throws MessagingException {
    val message = mailSender.createMimeMessage();
    val helper = new MimeMessageHelper(message, true, "UTF-8");
